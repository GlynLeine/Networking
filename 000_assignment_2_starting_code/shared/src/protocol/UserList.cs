    ...
    public class UserList : Protocol
    {
        ...
        protected override void Read(Packet packet)
        {
            ...
            for (uint i = 0; i < userCount; i++)
            {
                ...
                clientData.skinId = packet.ReadUint();
                clientData.x = packet.ReadFloat();
                clientData.y = packet.ReadFloat();
                data.Add(clientData);
            }
        }
        ...
        protected override void Write(Packet packet)
        {
            ...
            foreach(ClientData clientData in data)
            {
                ...
                packet.Write(clientData.skinId);
                packet.Write(clientData.x);
                packet.Write(clientData.y);
            }
        }
